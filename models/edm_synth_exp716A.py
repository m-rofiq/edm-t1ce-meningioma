...
class EDMSynth(nn.Module):
    def __init__(self, base_channels=32, in_channels=None, use_t2=True, use_flair=True):
        ...
        # MAIN ENCODER (T1)
        # ==========================================
        self.e1 = ResBlock(3, c)
        self.p1 = nn.MaxPool2d(2)

        self.e2 = ResBlock(c, c*2)
        self.p2 = nn.MaxPool2d(2)

        self.e3 = ResBlock(c*2, c*4)
        self.p3 = nn.MaxPool2d(2)

        self.e4 = ResBlock(c*4, c*8)
        ...
        if self.has_cond:
            self.ce1 = ResBlock(cond_ch, c)
            self.cp1 = nn.MaxPool2d(2)

            self.ce2 = ResBlock(c, c*2)
            self.cp2 = nn.MaxPool2d(2)

            self.ce3 = ResBlock(c*2, c*4)
            self.cp3 = nn.MaxPool2d(2)

            self.ce4 = ResBlock(c*4, c*8)
            self.cp4 = nn.MaxPool2d(2)
        ...
        self.base_head = nn.Conv2d(c, 1, 1)
        self.enh_head  = nn.Conv2d(c, 1, 1)
    ...
    def forward(self, x):
        ...
        base = self.base_head(d1)
        enh  = torch.relu(self.enh_head(d1))

        pred = torch.clamp(base + enh, 0.0, 1.0)

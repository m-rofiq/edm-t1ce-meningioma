...
class EDMSynth(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.has_cond and cond is not None:
            enh_raw = self.enh_net(cond)
            #enh = torch.relu(enh_raw) # 🔥 KUNCI: Penyangatan HANYA boleh positif!
            enh = F.softplus(enh_raw)
        else:
            # Jika tidak ada kondisi (hanya T1), enhancement = 0

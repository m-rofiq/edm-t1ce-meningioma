...
# Kritis untuk membuktikan perbaikan blur antar
# iterasi eksperimen.
#
# sharpness_tenengrad:
...
def main():
    ...
    for f in tqdm(files):
        ...
        # [NOTE] Variabel diganti dari 'ssim' → 'ssim_val' untuk
        # menghindari shadowing nama fungsi ssim dari pytorch_msssim
        # jika sewaktu-waktu diimport di scope yang sama.
        ...
        # [NEW] Sharpness metrics (global, inside brain mask)
        ...
        if torch.sum(enh_mask) > 10:
            ...
            # [NEW] LPIPS di enhancement region secara spesifik
            # Mengukur perceptual sharpness di area tumor —
            # indikator paling sensitif untuk mendeteksi blur
            # residual di region yang paling klinically relevant.
            with torch.no_grad():
            ...
            # [NEW] Sharpness ratio di enhancement region
        ...
        records.append({
            ...
            "lpips_enh"         : lpips_enh_val,   # [NEW]

            # Sharpness
            "sharpness_pred"    : sharp_pred,       # [NEW] nilai absolut
            "sharpness_gt"      : sharp_gt,         # [NEW] referensi GT
            "sharpness_ratio"   : sharp_ratio,      # [NEW] pred/gt, ideal=1.0
            "sharpness_ratio_enh": sharp_ratio_enh, # [NEW] khusus tumor region
        })
    ...
    for m in metrics:
        ...
        # [FIX] Guard: jika semua nilai NaN (misal tidak ada
        # enhancement region di seluruh holdout set), skip
        # bootstrap agar tidak crash.
        if len(vals) == 0:
    ...
    # Grouping print agar mudah dibaca
    groups = {
        "── Basic": [

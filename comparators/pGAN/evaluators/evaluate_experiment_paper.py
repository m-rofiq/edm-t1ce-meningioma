# DESTINATION: sejajar evaluate_experiment_paper.py Anda (mis. evaluators/../
# atau folder script eval Anda -- ikuti lokasi file aslinya)
#
# INI ADALAH evaluate_experiment_paper.py ANDA APA ADANYA, hanya DIPATCH pada
# SATU bagian: blok "LOAD MODEL". Tidak ada baris lain yang diubah -- semua
# metrik (psnr_roi, ssim_roi, calibration_bias, variance_ratio, sharpness,
# enhancement mask, bootstrap CI, profiling params/FLOPs/inference-time)
# 100% reused karena semuanya generik terhadap output tensor model, tidak
# bergantung pada arsitektur spesifik.
#
# KENAPA PERLU DIPATCH (satu-satunya blocker):
#   Baris asli:
#     model = MODEL_REGISTRY[model_name](
#         base_channels=CONFIG["base_channels"],
#         in_channels = 3 * len(CONFIG["modalities"])
#     ).to(device)
#   ini akan GAGAL untuk pGAN karena dua alasan:
#     1) "pGAN" belum terdaftar di MODEL_REGISTRY (KeyError)
#     2) constructor PGANGenerator pakai nama argumen input_nc/ngf/n_blocks,
#        BUKAN in_channels/base_channels seperti ResUNet/EDMSynth/dst.
#        (CONFIG["base_channels"] juga tidak ada di config_pGAN.py -- saya
#        pakai key "ngf" di sana, sesuai konvensi resmi pGAN)
#
# Solusi: tambah SATU cabang `elif model_name == "pGAN":` sebelum branch
# else generik -- pola ini sama persis dengan cabang khusus DMECNetStep yang
# sudah ada di script asli Anda.
...
# ==================================================
# SHARPNESS METRICS (identik dgn evaluate_experiment_paper.py)
# ==================================================
...
def sharpness_ratio(pred: torch.Tensor, gt: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
...
def main():
    ...
    if model_name in ["DMECNetStep1", "DMECNetStep2", "DMECNetStep3"]:
        ...
        # PGANGenerator pakai constructor resmi pGAN (input_nc/ngf/n_blocks),
        # bukan in_channels/base_channels seperti baseline U-Net Anda, dan
        # belum didaftarkan ke MODEL_REGISTRY -- jadi diimpor & diinstansiasi
        # langsung di sini, dgn kwargs diambil dari config_pGAN.py Anda.

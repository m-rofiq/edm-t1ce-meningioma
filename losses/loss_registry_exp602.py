...
class EDMLoss(torch.nn.Module):
    def __init__(self, lambda_enh=1.0, lambda_base=1.0): # Kembalikan weight ke 1.0
        super().__init__()
    ...
    def forward(self, pred_tuple, target, mask, input_tensor):
        ...
        # =========================
        # 🔥 SMART BASE CONSTRAINT
        # =========================
        # Definisikan area sehat: Di mana target (T1CE) tidak jauh lebih terang dari T1
        diff_t1ce_t1 = target - t1
        healthy_mask = (diff_t1ce_t1 <= 0.025) & mask
        tumor_mask = (diff_t1ce_t1 > 0.025) & mask

        # Paksa base_net HANYA belajar anatomi T1CE di area sehat!
        ...
        # =========================
        # 🔥 PURE ENHANCEMENT CONSTRAINT
        # =========================
        # Target enh adalah selisih dari target asli dikurangi anatomi base (yg sudah dipisahkan)
        enh_gt = target - base.detach()
        enh_gt = torch.clamp(enh_gt, min=0) # GT tidak boleh negatif
